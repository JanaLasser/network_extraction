...
def otsuThreshold(image):
    ...
    #np.histogramm doesn't create a full list with 256 entries if the last
    #entries are 0, need to append zeroes so the loops in the body of the 
    ...
    #main loop iterates over all the bins, calculating the between class
    #variance. The maximum of the between class variance is the minimum of the
    #in class variance so the threshold at the maximum is selected
    while(t < 256):
        t += 1
        ...
        varBetween = float(wB)*float(wF)*(float(mB) - float(mF))*\
                    (float(mB) - float(mF))
